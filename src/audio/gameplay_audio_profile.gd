...
@export var dispute: AudioStream
@export var alert_clear: AudioStream
@export var neutralized: AudioStream
@export var tactical_dash: AudioStream
@export var theft: AudioStream
@export var contact_hit: AudioStream
...
@export var cue_volume_offsets_db: Dictionary = {&"threat": 5.0, &"aggression": 2.0}
@export_range(0.1, 4.0, 0.05) var pitch_scale: float = 1.0
@export var retrigger_cue_ids: Array[StringName] = [&"fire"]
@export var coalesced_cue_ids: Array[StringName] = [&"dispute", &"threat", &"aggression", &"surrender", &"alert_clear", &"neutralized"]
@export_range(0.0, 1.0, 0.01) var coalesce_seconds: float = 0.12
...
func stream_for(cue_id: StringName) -> AudioStream:
	match cue_id:
		...
		&"alert_clear": return alert_clear
		&"neutralized": return neutralized
		&"tactical_dash": return tactical_dash
		&"theft": return theft
		&"contact_hit": return contact_hit
