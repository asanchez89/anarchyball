...
func is_structurally_valid() -> bool:
	return (
		...
		and surrender_resolve_ratio >= 0.0
		and surrender_resolve_ratio < 1.0
		...
		and ((is_zero_approx(patrol_distance) and is_zero_approx(patrol_speed)) or (patrol_distance > 0.0 and patrol_speed > 0.0))
		and telegraph_delay > 0.0
		and commitment_impact_delay >= 0.0
		and dialogue_lines.all(func(line: String) -> bool: return not line.strip_edges().is_empty())
	)
